...
public class AWSTest {
	    ...
	    private static final String AWSDNS = "databasedmp.csrbipfghawt.us-east-1.rds.amazonaws.com";
	    ...
	    public static void main(String args[]) {
	        ...
	        try (Connection con = DriverManager.getConnection("jdbc:mysql://" +
	            ...
	            System.out.print("Elige un número de película y te mostraré los personajes: ");
	            String numPelicula = sc.nextLine();

	            PreparedStatement pStmt = con.prepareStatement(
	                "SELECT C.id, name, C.height, C.mass, C.hair_color, C.skin_color " +
	                "FROM starwars.characters C, starwars.character_films CF, starwars.films F " +
	                "WHERE F.id=CF.id_film AND CF.id_character=C.id AND CF.id_film = ?;");
